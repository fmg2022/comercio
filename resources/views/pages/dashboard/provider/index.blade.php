  ...
  <x-tables.table>
    <x-slot:thead>
      <tr class="text-left">
        ...
        <th>Teléfono</th>
        <th class="hidden sm:table-cell">Cantidad Productos</th>
        <th>Estado</th>
    ...
    @forelse ($providers as $index => $provider)
      <tr {{ $provider->trashed() ? 'data-trash' : '' }}
        ...
        <td class="font-bold">{{ $provider->name }}</td>
        <td class="hidden lg:table-cell">{{ $provider->email }}</td>
        <td>{{ $provider->phone }}</td>
        <td class="text-center hidden sm:table-cell">{{ $provider->products->count() }}</td>
      ...
      <form enctype="multipart/form-data" method="POST"
        ...
        <fieldset class="py-3 grid grid-cols-1 gap-6 text-gray-700 md:px-3">
          ...
          <section
            ...
            <x-inputs.checkSwitch name="active" class="bg-slate-200 checked:bg-green-700 group-[.editable]:cursor-pointer"
