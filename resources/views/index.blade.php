...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased dark:bg-slate-900 dark:text-white/50">
        <header x-data="{ open: false }" class="relative py-6 px-3 w-full grid grid-cols-2 gap-4">
            <!-- Logos -->
            ...
            <!-- Links -->
            <nav :class="{'flex flex-1 justify-between ': open, 'hidden': !open}" class="absolute top-16 left-0 right-0 sm:static sm:flex pb-4 transition-all">
                <x-nav-ancor>Contacto</x-nav-ancor>
                <x-responsive-nav-link :href="'#'">Nav algo</x-responsive-nav-link>
                @if (Route::has('login'))
                    @auth
                ...
                <div :class="{'block': open, 'hidden': ! open}" class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600 hidden sm:hidden">
                    ...
                    <div class="mt-3 space-y-1">
                        <x-responsive-nav-link :href="'#'">Nav algo</x-responsive-nav-link>
                        <x-responsive-nav-link :href="'#'">Nav algo2</x-responsive-nav-link>
                    </div>
                </div>
            </nav>
            <!-- Hamburger -->
